...
def test_cross_entropy(N, hidden_size, vocab_size, provider):
    ...
    if provider == "pytorch":
        ...
        def run_standard():
            ...
            loss = standard_ce(logits.view(-1, vocab_size), labels.view(-1))
            loss_sum = loss.sum()  # Sum to get a scalar for backward
            loss_sum.backward()
            return loss
        ...
        def run_triton():
            loss = cross_entropy_fused_triton_old(
        ...
        def run_triton_acc():
            loss = cross_entropy_fused_triton(
                hidden_states_clone.view(-1, hidden_states_clone.size(-1)),
                lm_head_weight_clone,
                labels_clone.view(-1, 1),
            )
...
if __name__ == "__main__":
    ...
    @triton.testing.perf_report(
        triton.testing.Benchmark(
            ...
            ylabel="ms",
            plot_name="Cross Entropy Forward+Backward (hidden_size=2048)",
    ...
    #         styles=[("blue", "-"), ("green", "--"), ("red", "--")],
    #         ylabel="ms",
    #         plot_name="Cross Entropy Forward+Backward (hidden_size=1024)",
