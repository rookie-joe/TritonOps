...
def attention_kernel(
    ...
    for i in range(0, tl.cdiv(N_CTX, BLOCK_SIZE_N)):
        ...
        last_max_qk = max_qk
        max_qk = tl.maximum(
            last_max_qk, tl.max(qk, axis=1)
        )  # 这里的 axis 不能写 -1 之类的，不然编译会失败
        # 计算 exp(qk-max_qk)
        exp_qk = tl.exp(qk - max_qk[:, None])  # 注意 max_qk 的维度
        # 计算 sum(exp(qk-max_qk))
        last_sum_exp = sum_exp
        sum_exp = last_sum_exp * tl.exp(last_max_qk - max_qk) + tl.sum(exp_qk, axis=1)
        # 更新 output
        o = o * (tl.exp(last_max_qk - max_qk) * last_sum_exp / sum_exp)[:, None]
        v = tl.load(v_ptrs, kv_mask, 0)
        o = o + tl.dot(exp_qk.to(v.dtype), v) / sum_exp[:, None]
        # 指针移动一个 block
